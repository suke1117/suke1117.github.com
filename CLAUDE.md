...
8. **探索と特徴量の判定は 1 回の測定で決めない**:
   - パラメータ探索の成績は、同じグリッドを「市場がちょうど正しい」ヌル世界でも回し、
     そこでの最良点とグリッド平均の差を選択バイアスとして差し引いてから報告する (`sweep.py --null_runs`)。
     **ヌルの基準は break-even ではなく 1-控除率**。100% と比べると控除率を上振れと取り違える。
   - 特徴量 family の採否は複数ウィンドウで測る (`ablation.py --windows`)。有意と言えるのは、
     平均がウィンドウ分散の 2 倍を超え、かつ全ウィンドウで符号が揃うときだけ。
9. **成績の報告には必ず区間を付ける**:
...
6. **画面に識別子を出さない**: 馬と騎手は `explain.runner_label` が返す「馬番」または「馬番 名前」で呼ぶ。
   血統登録番号 (H00918) や騎手コード (J055) を画面・文章・配布用テキストに出してはならない。
   馬番は馬の番号であり、実際に賭けるときに入力する番号でもある。
   名前は `entrant_name` / `jockey_name` が**データにある場合だけ**使う。**生成してはならない**。
   実在の数字が並ぶページに架空の馬名を置けば、実在の馬として読まれる。
   名前の列は、その日のデータに名前がある場合だけ表示する (「—」で埋めた列は無い方がよい)。
   専門用語を出す場合は平易な語を主、術語を括弧内の従とする (「勝率に変換 (PL)」)。
