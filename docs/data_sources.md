...
**確認できたこと (公開実装経由、実ファイルは未確認)**: 月次 ZIP の中身は
`{YYYYMM}_racelist.csv` (66列) / `{YYYYMM}_horselist.csv` (36列) / `{YYYYMM}_payback.csv` (54列)、
CP932。**着順は horselist に含まれます** — これが最大の懸念でしたが、解消しました。
オッズは別の ZIP (`OddsDataDownload`)。対応表のたたき台は `docs/mapping.nar.example.yml`。

**この形式には構造的な問題が 2 つあります。**

1. **race_id が無い**。競馬場 + 競走年月日 + レース番号 で合成するしかありません
   (`mapping.yml` の `derive:` がこれを行います)。
2. **馬ID・騎手IDが無く、名前しかない**。as-of 集計はすべて主体の ID で束ねるので、
   名前を代用することになります。地方競馬には改名と同名馬が実在するため、
   「別の馬を同じ馬として集計する」と「同じ馬を別の馬として分断する」が両方起きます。
   `--check` が警告しますが、警告で消える問題ではありません。
   本格的にやるなら (馬名 + 生年月日 + 父馬名) のハッシュを代理キーにして
   改名を名寄せする必要があります。**過去成績特徴量の精度を直接決めるので、
   実データが入ったら最初に検証すべき箇所です。**

もう 1 つ、リークの罠があります。horselist には着順・タイム・着差・上がり3F・人気に加えて、
騎手成績・全成績・当競馬場成績など**主体の累積成績 8 列**が入っています。
これらが「当該レースを含む時点」なのか「直前まで」なのかは公開仕様から読み取れません。
含んでいれば特徴量にした瞬間リークです。このシステムは過去成績を履歴から自分で
as-of 集計するので元から不要で、対応づけなければ済みます。`--check` がこれも警告します。

**未確認の点**: このツールの実行環境から keiba.go.jp に到達できないため、実ファイルは見ていません
(環境の Network access が **Trusted** で、許可ドメインに入っていない)。
1 か月分の ZIP を `raw_data/` に展開して `python src/data/preprocess.py --check` を打てば、
何が取れて何が足りないかが一度に出ます。
...
derive:
  race_id: [競馬場, 競走年月日, レース番号]   # キー列が無いファイル用。値をつないで作る
races:
  開催年月日: race_date       # <CSVの見出し>: <正規化後の名前>
  競馬場コード: venue
entries:
  馬番: post_position
  馬名: [entrant_id, entrant_name]   # 1 列を 2 つの役割に使う (ID が無いファイル用)
values:
  surface: {"芝": turf, "ダ": dirt}   # 芝/ダ/良/稍重/重/不良 は既定で変換済み
```

頭数 (`n_runners`) の列が無ければ、出馬表の行数から数えます。書く必要はありません。
